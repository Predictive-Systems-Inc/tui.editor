...
에디터 사용 방식은 기존 v2.x와 동일하게 [스코프드 패키지(Scoped package)](https://docs.npmjs.com/using-npm/scope.html)를 적용하여 다음과 같이 `@predictive-systems/editor`로 패키지를 설치하여 사용한다. 아래는 npm 커맨드를 사용한 에디터 설치 예제이다.
...
```js
const Editor = require('@predictive-systems/editor'); /* CommonJS 방식 */
import Editor from '@predictive-systems/editor'; /* ES6 모듈 방식 */
```
...
```js
import { EditorCore } from '@predictive-systems/editor'; /* ES6 모듈 방식 */
...
v3.0부터는 jQuery Wrapper가 제거되었다. jQuery에서 사용을 원하는 경우 직접 `@predictive-systems/editor` 패키지를 랩핑하여 사용해야 한다.
